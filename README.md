...
- 对照原文直接编辑译文，比较并采用历史版本；单段重译先预览再采用，人工修改会受到批量润色和批量重译保护。
- 将完成的项目导出为 TXT 或 EPUB，支持仅译文和逐段双语对照，并同步译文标题与目录；[导出用法与验收记录](docs/bilingual-export.md)。
...
- [ ] 在保证章节顺序、上下文和术语一致性的前提下，通过可控的并行翻译与并行重译提升处理速度。
- [ ] 支持术语表与翻译记忆的导入、导出和跨项目复用，方便同系列作品保持译名统一。
- [ ] 增加项目备份和迁移，降低长篇翻译过程中的修改与恢复成本。
