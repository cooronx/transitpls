...
interface Project { id:string; title:string; source_file:string; source_path:string; source_language:string; target_language:string; status:Status; chapters_total:number; chapters_completed:number; updated_at:string; cover_data_url?:string|null; task_initialized?:boolean }
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    listen<Detail>("translation-progress",({payload})=>{
      ...
      setBootstrap((current)=>current?{...current,projects:current.projects.map((project)=>project.id===payload.project.id?{...payload.project,cover_data_url:project.cover_data_url,task_initialized:project.task_initialized}:project)}:current);
  ...
  const importFile = async () => {
    ...
    if(path) await run("导入书籍",async()=>{
      try{return await invoke<Detail>("ui_import",{input:path})}
      catch(error){
  ...
  const saveGeneral = async (visibleSegments:number) => {
    setBusy("保存通用设置");setNotice(null);
    try{const next=await invoke<Bootstrap>("ui_save_general",{visibleSegments});setBootstrap(next);setNotice("通用设置已保存")}
    catch(error){setNotice(String(error))}finally{setBusy(null)}
  };
  const savePipeline = async (polish:boolean) => {
    if(busy)return;
    setBusy("保存润色设置");setNotice(null);
    try{const next=await invoke<Bootstrap>("ui_save_pipeline",{polish});setBootstrap(next);setNotice(`译后润色已${polish?"开启":"关闭"}`)}
    catch(error){setNotice(String(error))}finally{setBusy(null)}
  };
  ...
  const matchingSegments=useMemo(()=>{if(!chapter)return[];const q=search.trim().toLowerCase();return q?chapter.segments.filter((s)=>s.source.toLowerCase().includes(q)||s.target?.toLowerCase().includes(q)):chapter.segments},[chapter,search]);
  const segments=useMemo(()=>matchingSegments.slice(0,displaySegmentCount),[matchingSegments,displaySegmentCount]);
  useEffect(()=>{setDisplaySegmentCount(visibleSegmentCount)},[chapterIndex,search,visibleSegmentCount]);

  return <div className="app-shell">
    <Header project={detail?.project} projects={bootstrap?.projects??[]} model={bootstrap?.config.llm.model??"—"} search={search} onSearch={setSearch} onProject={(id)=>void selectProject(id)} onTranslate={()=>translate()} busy={busy} onCancel={()=>void cancelTask()} disabled={!detail?.taskInitialized}/>
    <div className="workspace-row">
      ...
      <main className="main-panel">
        {view==="workspace"&&detail&&chapter&&<Workspace detail={detail} chapter={chapter} chapterIndex={chapterIndex} segments={segments} hasMoreSegments={matchingSegments.length>segments.length} onShowMore={()=>setDisplaySegmentCount((count)=>count+visibleSegmentCount)} tray={tray} setTray={setTray} translating={busy==="翻译"} ready={detail.taskInitialized} onTranslate={()=>translate(chapterIndex)} onExport={exportBook}/>}
        ...
        {view==="settings"&&<SettingsView config={bootstrap?.config} credential={bootstrap?.credential} configPath={bootstrap?.configPath} busy={busy} onSaveModel={saveModel} onSaveGeneral={saveGeneral}/>}
...
function Workspace({detail,chapter,chapterIndex,segments,hasMoreSegments,onShowMore,tray,setTray,translating,ready,onTranslate,onExport}:{detail:Detail;chapter:Chapter;chapterIndex:number;segments:Segment[];hasMoreSegments:boolean;onShowMore:()=>void;tray:TrayName;setTray:(t:TrayName)=>void;translating:boolean;ready:boolean;onTranslate:()=>void;onExport:(f:"txt"|"epub")=>void}){
  return <div className="editor-layout"><div className="editor-tabs"><button className="active icon-label"><Columns2/>对照翻译</button><button className="icon-label"><ListTree/>结构预览</button><button className="icon-label" onClick={()=>setTray("issues")}><ShieldCheck/>质量检查</button><span/><small>第 {chapterIndex+1} / {detail.chapters.length} 章</small><button className="run-chapter icon-label" disabled={translating||!ready} onClick={onTranslate}>{translating?<LoaderCircle className="spin"/>:<Play/>}{translating?"正在翻译":ready?"翻译本章":"请先初始化"}</button></div><div className="breadcrumb"><FileText/>{fileName(detail.project.source_file)}<i>/</i><b>{chapter.target_title||chapter.title}</b><em>{chapter.segments.length} 个段落</em></div><div className="column-head"><div><b>原文</b><span>{languageName(detail.project.source_language)}</span></div><div><b>译文</b><span>{languageName(detail.project.target_language)}</span></div></div><section className="segments">{segments.length?segments.map((s)=><SegmentCard key={s.id} segment={s}/>):<div className="no-results">没有匹配的段落</div>}{hasMoreSegments&&<button className="load-more-segments icon-label" onClick={onShowMore}><ChevronDown/>显示更多段落</button>}</section><Tray detail={detail} tray={tray} setTray={setTray} onExport={onExport}/></div>
}
...
function TermsView({detail,onReload}:{detail:Detail|null;onReload:()=>Promise<void>}){const[editing,setEditing]=useState<string|null>(null),[target,setTarget]=useState("");const resolve=async(term:Term)=>{if(!detail||!target.trim())return;await invoke("ui_resolve_term",{projectId:detail.project.id,source:term.source,target});setEditing(null);await onReload()};return <div className="page-view"><header><div><h1>术语库</h1><p>{detail?`${detail.project.title} · ${detail.terms.length} 条术语`:"选择项目后查看术语"}</p></div></header>{detail&&detail.terms.length?<div className="term-table"><div className="term-row term-head"><span>原文</span><span>固定译名</span><span>类型</span><span>首次出现</span><span>状态</span><span>操作</span></div>{detail.terms.map((term)=><div className="term-row" key={term.source}><b>{term.source}</b>{editing===term.source?<input autoFocus value={target} onChange={(e)=>setTarget(e.target.value)}/>:<span>{term.target}</span>}<span>{termTypeText[term.type]??"其他"}</span><span>第 {term.first_chapter+1} 章</span><em className={term.status}>{term.status==="conflict"?"有冲突":term.status==="resolved"?"已裁定":"正常"}</em>{editing===term.source?<button onClick={()=>void resolve(term)}>保存</button>:<button onClick={()=>{setEditing(term.source);setTarget(term.target)}}>修改</button>}</div>)}</div>:<div className="page-empty">暂无术语数据</div>}</div>}
function SettingsView({config,credential,configPath,busy,onSaveModel,onSaveGeneral}:{config?:Config;credential?:CredentialStatus;configPath?:string;busy:string|null;onSaveModel:(value:Config,key:string)=>Promise<void>;onSaveGeneral:(visibleSegments:number)=>Promise<void>}){
  const[tab,setTab]=useState<"model"|"general">("model");
  const[draft,setDraft]=useState(config);
  const[visibleSegments,setVisibleSegments]=useState(config?.general.visible_segments??100);
  const[apiKey,setApiKey]=useState("");
  const[showKey,setShowKey]=useState(false);
  useEffect(()=>{setDraft(config);setVisibleSegments(config?.general.visible_segments??100)},[config]);
  if(!draft)return <div className="page-empty">正在读取设置…</div>;
  const field=(key:keyof Config["llm"],value:string)=>setDraft({...draft,llm:{...draft.llm,[key]:value}});
  ...
  return <div className="page-view model-page"><header><div><h1>设置</h1><p>配置翻译模型与连接凭据。</p></div></header><div className="settings-layout"><nav><button className={tab==="model"?"active":""} onClick={()=>setTab("model")}><Braces/>模型与 API</button><button className={tab==="general"?"active":""} onClick={()=>setTab("general")}><PanelTop/>通用</button><button className="icon-label" disabled><Settings/>高级</button></nav>{tab==="model"?<div className="settings-card"><div className="settings-heading"><div><h2>模型配置</h2><p>选择模型提供商，并验证用于翻译的 API Key。</p></div>{configured&&<span className="credential-ok icon-label"><Check/>密钥已保存 ····{credential?.lastFour}</span>}</div><Field label="接口格式"><select value={draft.llm.provider} onChange={(e)=>{field("provider",e.target.value);setApiKey("")}}><option value="openai-chat">OpenAI Chat Completions(OpenAI 兼容)</option><option value="openai-responses">OpenAI Responses</option><option value="anthropic">Anthropic(Messages)</option></select></Field><Field label="模型"><input value={draft.llm.model} onChange={(e)=>field("model",e.target.value)}/></Field><Field label="API地址(Base URL)"><input value={draft.llm.base_url??""} onChange={(e)=>field("base_url",e.target.value)}/></Field><Field label="API Key"><div className="secret-input"><input autoFocus={!credential?.configured} type={showKey?"text":"password"} value={apiKey} onChange={(e)=>setApiKey(e.target.value)} placeholder={configured?`已保存 ····${credential?.lastFour}，留空则不更改`:"粘贴 API Key"}/><button type="button" onClick={()=>setShowKey(!showKey)}>{showKey?"隐藏":"显示"}</button></div><small>Key 仅保存在当前用户的 TransItPls 配置目录中，不会写入书籍项目。</small></Field>{credential?.source==="environment"&&sameProvider&&<div className="config-note">当前优先使用环境变量 {draft.llm.api_key_env}；桌面端保存的 Key 将作为后备。</div>}<div className="settings-actions"><button className="primary" disabled={busy!==null||(!configured&&!apiKey.trim())} onClick={()=>void onSaveModel(draft,apiKey)}>{savingModel?"正在验证…":"测试连接并保存"}</button></div><footer>模型配置：<code>{configPath??"保存后创建 transitpls.toml"}</code></footer></div>:<div className="settings-card"><div className="settings-heading"><div><h2>通用设置</h2><p>调整工作台中每个章节默认加载的段落数量。</p></div></div><Field label="每章默认显示段落数"><input type="number" min="1" step="1" value={visibleSegments} onChange={(e)=>setVisibleSegments(Number(e.target.value))}/><small>章节切换时先显示这些段落，点击“显示更多段落”可继续查看其余内容。</small></Field><div className="settings-actions"><button className="primary" disabled={busy!==null||visibleSegments<1||!Number.isInteger(visibleSegments)} onClick={()=>void onSaveGeneral(visibleSegments)}>{savingGeneral?"正在保存…":"保存通用设置"}</button></div><footer>配置文件：<code>{configPath??"保存后创建 transitpls.toml"}</code></footer></div>}</div></div>
}
